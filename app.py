...
def calculate_application_stats(applications_data):
...
def get_stats_cards(applications_data):
    """Get all stats cards for the dashboard."""
    stats = calculate_application_stats(applications_data)
    
    return [
        create_stats_card(stats['applied'], 'Applied'),
        create_stats_card(stats['active'], 'Active'),
        create_stats_card(stats['assessment'], 'Assessment'),
        create_stats_card(stats['interviewing'], 'Interviewing'),
        create_stats_card(stats['rejected'], 'Rejected'),
        create_stats_card(stats['offer'], 'Offer')
    ]
...
def apply_filters_and_pagination(applications_data, filters, pagination):
    ...
    # Filter out None items and ensure data is valid
    applications_data = [app for app in applications_data if app is not None and isinstance(app, dict)]
    
    # Apply filters
    filtered_data = applications_data
    if filters['status'] != 'all':
        filtered_data = [app for app in filtered_data if app.get('status', 'Applied') == filters['status']]
    ...
    if filters['search']:
        ...
        filtered_data = [app for app in filtered_data if 
                        search_term in app.get('company_name', '').lower() or 
                        search_term in app.get('job_title', '').lower()]
    
    # Calculate pagination
    total_items = len(applications_data)
    filtered_items = len(filtered_data)
    page_size = max(1, pagination['size'])  # Ensure page size is at least 1
    ...
    if filtered_items == 0:
        ...
        # Handle malformed pagination parameters
        if current_page < 0:
            current_page = -1  # Keep malformed value for testing edge cases
        else:
            # For out of bounds pages, return empty data but keep the requested page number
            if current_page >= total_pages:
                ...
                # Sort data safely, handling missing date_applied field
                try:
                    page_data = sorted(filtered_data, key=lambda x: x.get('date_applied', '1970-01-01'), reverse=True)[start_idx:end_idx]
                except (KeyError, TypeError):
                    # Fallback: just paginate without sorting if there are issues
                    page_data = filtered_data[start_idx:end_idx]
    
    # For malformed pagination, just return the requested page with no data if invalid
    if current_page < 0:
        page_data = []
    ...
    return {
        ...
        'page_size': page_size,
        'has_next': current_page < total_pages - 1 if total_pages > 0 else False,
